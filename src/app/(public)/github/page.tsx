...
export default async function GitHubPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-32 w-full">
      {/* Header */}
      <div className="mb-16">
        <h2 className="text-sm uppercase tracking-widest text-muted font-bold mb-4">
          /01 Code &amp; Scripts
        </h2>
      ...
      {/* Profile Card */}
      <div className="glass-card p-8 mb-16 flex flex-col md:flex-row items-center gap-8">
        <div className="w-24 h-24 rounded-full overflow-hidden flex-shrink-0 border-4 border-border">
          <Image
        ...
        <div className="flex-1 text-center md:text-left">
          <h2 className="text-2xl font-bold">{ghProfile?.name || 'Bimsara Gunawardana'}</h2>
          <a
          ...
          {ghProfile && (
            <div className="flex justify-center md:justify-start gap-6 mt-3 text-sm">
              <span>
                <strong>{ghProfile.public_repos}</strong> <span className="text-muted">repos</span>
              </span>
              <span>
                <strong>{ghProfile.followers}</strong> <span className="text-muted">followers</span>
              </span>
              <span>
                <strong>{ghProfile.following}</strong> <span className="text-muted">following</span>
              </span>
            </div>
          )}
        </div>
      ...
      <div className="glass-card p-8 mb-16 overflow-x-auto">
        <h3 className="text-xl font-bold mb-6">Contribution Activity</h3>
        <GitHubCalendar username="bimsara0608" colorScheme="light" blockSize={13} blockMargin={4} />
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {repos.map((repo) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="glass-card p-6 block group hover:-translate-y-1 transition-transform"
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <ArrowRight
                    size={18}
                    className="text-gray-300 group-hover:text-accent transition-colors flex-shrink-0"
                  />
                </div>
