...
export function EditProjectForm({ project }: { project: Project }) {
  ...
  return (
    <div className="max-w-3xl">
      <Link
        href="/admin/projects"
        className="magnetic inline-flex items-center gap-2 text-muted-foreground hover:text-foreground font-medium mb-8 transition-colors"
      >
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        <div className="bg-card border border-border rounded-xl p-6 space-y-5">
          <h2 className="text-lg font-bold border-b border-border/50 pb-3">Basic Information</h2>
          <div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-3 bg-muted border border-border font-medium text-sm rounded-lg hover:bg-muted/80 transition-colors"
              >
                Auto-generate
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input {...register('client')} className={fieldClass} />
            </div>
            <div>
              <label className={labelClass}>Timeline</label>
              <input {...register('timeline')} className={fieldClass} />
            </div>
          </div>
          <div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-5">
          <h2 className="text-lg font-bold border-b border-border/50 pb-3">Project Content</h2>
          <div>
          ...
          <div>
            ...
            <textarea {...register('challenge')} rows={4} className={`${fieldClass} resize-none`} />
          </div>
          <div>
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          <h2 className="text-lg font-bold border-b border-border/50 pb-3">Project Images</h2>
          <ImageUploader projectId={project.id} existingImages={images} onChange={setImages} />
        </div>

        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          <h2 className="text-lg font-bold border-b border-border/50 pb-3">
            3D Model (Interactive Viewer)
          </h2>
          <p className="text-sm text-muted-foreground">
            Upload a .glb or .gltf file to enable the interactive 3D model viewer on this project
            page.
          </p>
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          <h2 className="text-lg font-bold border-b border-border/50 pb-3">Publish Settings</h2>
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <div>
              <p className="font-bold">Publish publicly</p>
              <p className="text-sm text-muted-foreground">Make visible on portfolio</p>
            </div>
          </label>
          <label className="flex items-center gap-3 cursor-pointer">
            <input
        ...
        {error && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-500 px-4 py-3 rounded-lg font-medium">
            {error}
          </div>
        )}
        ...
        <div className="flex gap-3 justify-end">
          <Link
            href="/admin/projects"
            className="magnetic px-6 py-3 border border-border font-medium rounded-lg hover:bg-muted transition-colors"
          >
            Cancel
          </Link>
          <button
            ...
            className="magnetic bg-foreground text-background px-8 py-3 rounded-lg font-bold hover:opacity-90 transition-opacity flex items-center gap-2 disabled:opacity-70"
          >
            <Save size={18} />
